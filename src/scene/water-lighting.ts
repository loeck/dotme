...
vec3 waterBRDF(vec3 n, vec3 v, vec3 l, vec3 radiance) {
  ...
  return radiance * nl * (vec3(distribution * visibility * fresnel)
    + vec3(0.002, 0.004, 0.006));
}

vec3 waterLighting(vec3 worldNormal, vec3 worldView) {
  ...
  for (int i = 0; i < NUM_POINT_LIGHTS; i++) {
    ...
    #endif
    result += waterBRDF(n, v, light.direction, light.color) * visibility;
    }
  }
