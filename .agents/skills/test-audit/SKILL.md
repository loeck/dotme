...
whole subsystem's test surface (every test file an area such as `src/scene`
owns, plus the `e2e` specs that cover it); before starting one, read
...
routing, and relevant history. Read `docs/architecture.md` and the subsystem
doc under `docs/` first. When the test claims dependency-backed behavior,
...
- unit tests by area (`src/audio`, `src/components`, `src/scene`, `src/weather`);
- e2e specs (`e2e/`) across the three playwright configs;
- scripts and tooling (`scripts/`, configs, CI);
...
migration, storage, security, platform, default, bundle-size, generated,
...
Never edit source or tests while `pnpm test:watch` is running in the checkout.

1. Run the smallest owner and sibling unit tests with
   `pnpm vitest run <path-or-filter>`.
2. Run the affected e2e specs with the playwright config that routes them:
   `pnpm e2e <spec>`, `pnpm e2e:extended <spec>`, or `pnpm e2e:gpu <spec>`.
   Confirm each spec is actually routed by checking the `testMatch` lists in
   the three configs. Install browsers on first run with
   `pnpm exec playwright install chromium`.
3. For removed source greps or script assertions, run the owning executable
   under `scripts/` (e.g. `pnpm build && pnpm bundle:size` for loader-budget
   contracts).
4. Run targeted formatting and lint on changed files
   (`pnpm exec oxfmt <files>`, `pnpm exec oxlint --type-aware <files>`),
   then `git diff --check`.
5. Run `pnpm typecheck`.
6. Inspect `git diff --numstat`; report production/tooling separately from
   tests and test support.
7. After final audit edits, run the full gate as CI does:
   `pnpm check && pnpm e2e`.

## Landing and continuation

Commit, push, open a PR, or land only when authorized. Open PRs with
`gh pr create` and land via merge commit through the PR. Land one
